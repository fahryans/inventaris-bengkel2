        ...
        <div class="card-body">
            <form action="{{ route('alat.update', $alat) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_kategori" id="id_kategori" class="form-select @error('id_kategori') is-invalid @enderror" required>
                                ...
                                @foreach($kategoris as $kat)
                                    <option value="{{ $kat->id }}" {{ old('id_kategori', $alat->id_kategori) == $kat->id ? 'selected' : '' }}>
                                        {{ $kat->nama_kategori }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="spesifikasi" id="spesifikasi" class="form-control @error('spesifikasi') is-invalid @enderror" 
                              rows="3">{{ old('spesifikasi', $alat->spesifikasi) }}</textarea>
                    @error('spesifikasi')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select name="tipe_pelacakan" id="tipe_pelacakan" class="form-select @error('tipe_pelacakan') is-invalid @enderror" required>
                                ...
                                <option value="agregat" {{ old('tipe_pelacakan', $alat->tipe_pelacakan) == 'agregat' ? 'selected' : '' }}>Agregat</option>
                                <option value="unit" {{ old('tipe_pelacakan', $alat->tipe_pelacakan) == 'unit' ? 'selected' : '' }}>Unit</option>
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" name="jumlah_alat" id="jumlah_alat" class="form-control @error('jumlah_alat') is-invalid @enderror" 
                                   value="{{ old('jumlah_alat', $alat->jumlah_alat) }}" min="1" required>
                            @error('jumlah_alat')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
